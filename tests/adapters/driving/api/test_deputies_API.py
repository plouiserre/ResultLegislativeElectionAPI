...
def override_deputies_business() :
    mock = Mock()
    mock.get_deputies.return_value = [{"LastName" : "CARPENTIER", "FirstName" : "Julien", "Sexe" : "M"}, {"LastName" : "DUFREGNE", "FirstName" : "Jean-Paul", "Sexe" : "M"}, {"LastName" : "BENOIT-GOLA", "FirstName" : "Anne-Cécile", "Sexe" : "F"}] 
    mock.get_deputy_from_candidate_identity.return_value = [{"LastName" : "DOUARD", "FirstName" : "Dominique", "Sexe" : "F"}] 
    return mock
...
class DeputiesAPITest(unittest.TestCase) : 
    ...
    def test_get_deputies_from_candidates_status_OK_when_no_errors(self) : 
        self.overriding_business_dependency("success")
        response = self.client.get("/deputies/?cn_first_name=john&cn_last_name=doe")
        
        self.assertEqual(200, response.status_code)
        self.assertEqual([{"LastName" : "DOUARD", "FirstName" : "Dominique", "Sexe" : "F"}] , response.json())
